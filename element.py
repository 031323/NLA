...
A=None
B=None
def homogenousdirichlet(mesh,f,fixed,g=lambda x:0,h=lambda x:0):
    node_map=[]
    n_free_nodes=0
...
for i in range(0,L1):
    for j in range(0,L2):
        test_mesh.nodes.append([i/L1,j/L2])
        if (i!=0) and (j!=0):
            v00=L2*(i-1)+j-1
            v01=L2*(i-1)+j
            v11=L2*i+j
...
def test_h(i):
    ...
    if i<L2:
        ...
        return 0
solve=homogenousdirichlet(test_mesh,test_f,test_fixed,test_g,test_h)
